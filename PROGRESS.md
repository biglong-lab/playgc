...
- **免登入**：遊玩類路由自動建立訪客身分（GuestGate），拿掉登入牆 / 暱稱框
- **QR 直達**：`/g/:slug` 過場頁直接導向（單人第一關 / 組隊大廳 / 章節），套遊戲所屬場域；已印 QR 不用重印
- **開局減法**：直接接續 + 可反悔提示、GPS 只給地點鎖遊戲、遊玩中不跳全站彈窗、組隊只隊長可開始（倒數 3 秒）
- **結束保存紀錄**：結算頁引導登入，訪客紀錄認領到正式帳號（HMAC 憑證、16 表、冪等）
- **計分開關**：`games.scoring_enabled`（啟動冪等補欄位），不計分遊戲整個不顯示分數、不上排行榜
- **附帶修**：競賽 / 接力開賽導向、隊伍獎勵帶 userId、QR 管理頁重產短連結
- **驗證**：tsc 0 錯、vitest 3439 全過、新 e2e 2 支（單人 QR→保存、組隊雙手機）真瀏覽器通過；code-reviewer 初審 Block → 全修
- **待業主決定**：訪客刷券風險（獎勵是否限正式帳號）、競賽計分另案


### 📡 2026-07-22 MQTT／設備管理規劃（待實作）

完整計畫 → [docs/changes/2026-07-22-mqtt-device-integration-plan.md](docs/changes/2026-07-22-mqtt-device-integration-plan.md)

- 已盤點 `/admin/devices`、設備 API、MQTT service、資料表、測試與 ESP32 範例。
- 確認現有 UI 是管理骨架，尚有 MQTT 未啟動、Topic／payload／指令協定不一致、缺場域隔離與硬體 ID onboarding 等 P0 缺口。
- 決策方向：先凍結 MQTT v1 contract，再完成安全地基、gateway、管理頁、ESP32 韌體與遊戲 session lease。
- 驗證基線：`server/__tests__/devices.test.ts` 在允許本機臨時埠後 26/26 通過；目前僅為 mock API 測試。
- 下一步：Phase 0 — ADR + Topic builder/parser + Zod contract + simulator + local Mosquitto integration test。
- 預估：完整 7 階段約 11–14 個工作天；必須收到明確開發指令後才開始實作。

### 🛡️ 2026-05-31 安全/CI 硬化（本機，待部署）

完整紀錄 → [docs/changes/2026-05-31-repo-hygiene-security-hardening.md](docs/changes/2026-05-31-repo-hygiene-security-hardening.md)

- **Phase 0 止血**：DB dump / CI 產物移出版控 + gitignore
- **Phase 1 CI 可信度**：ESLint ignores（lint 噪音 16 萬→真實 345 errors）、CI 補 lint、retry CI 設 0（測試 547s→30s）
- **Phase 2 安全護欄**：logger 遮罩、health/detail 守門、IPv6 限速、deploy.sh 不自動收檔、seed 生產守門
- **驗證**：tsc 零錯誤；測試 53 failed（既有 mock 債、零 regression）
- **待議**：Phase 3 body 上限、Phase 4 歷史清 dump（凍結）、Phase 5 大檔拆分、Phase 6 LiveKit 鎖版本

---

### 📜 歷史進度

**2026-05-08（凌晨）** — 🎉 多人遊戲穩定性全套上線 + AdminMultiSessions v2 完整打磨（commit `550d60bc`，已生產部署）

### 🎯 2026-05-08 完整成果（單日 14h、8 commits、5700 行）

**全日總結** → [docs/changes/2026-05-08-summary.md](docs/changes/2026-05-08-summary.md)

針對 user 反映「進入遊戲斷線 / 對講機離線 / 猜謎不公平」、外加「監控頁排版 / 真實資料 / 完整紀錄」做架構級治本 + 監測打磨：

#### A. 多人遊戲穩定性重構（Phase 0-4 + e2e、已上線）
- **Phase 0.1-0.3** 完整監測體系（admin UI / 90 天 log / Session Replay）
- **Phase 1-3** 自寫 WS + 全域單例（4 條 ws → 1 條 + ADR-0018 + CI 規範）
- **Phase 4** TriviaShowdown server-side scoring（DB persistence、絕對公平）
- **e2e** 16 個真實多人 e2e（填補 CLAUDE.md 紅線第 10 條）

#### B. AdminMultiSessions v2 — 16 項完整打磨（已上線）
- **P0**（5 項）三列佈局 + 4 MetricCard + 真實 online + 健康指標 + 異常排序
- **P1**（4 項）篩選列 + 玩家詳情 + 迷你時間軸 + CSV 匯出
- **P2**（3 項）game 進度 + sessionId 複製 + refresh 倒數
- **P3**（4 項）Telegram 告警 cron + cross-session 歷史 + broadcast/min + 鍵盤導航

### 🛡️ 業主新增能力（生產可用）

1. **`/admin/multi-sessions`** — 三列即時連線監控、5s refresh、4 MetricCard、篩選/CSV
2. **`/admin/sessions/:id/replay`** — 完整時間軸 + 篩選 + CSV export（爭議仲裁）
3. **真實 online 狀態** — 三態（online/away/offline）、用 ws_event_log 不再 5 分鐘 polling
4. **健康指標** — grace / auto_leave / kick / error 過去 5 分鐘聚合、異常自動排頂
5. **Telegram 自動告警** — critical session（score≥20）→ notify + Replay 連結 + 30min cooldown
6. **玩家 cross-session 歷史** — 點頭像看 1/7/30/90 天連線統計 + 多 IP 偵測
7. **1 user = 1 條 ws** — 進入遊戲不斷線、對講機自動繼承重連
8. **猜謎絕對公平** — server-side scoring + DB persistence
9. **90 天事件 log** — 任何時間可查爭議
10. **CI 規範防回頭** — grep new WebSocket() 違規 CI 阻擋
...
|------|------|
| **全日總結** | [docs/changes/2026-05-08-summary.md](docs/changes/2026-05-08-summary.md) |
| 完整規劃 | [docs/changes/2026-05-08-multi-stability-refactor-plan.md](docs/changes/2026-05-08-multi-stability-refactor-plan.md) |
| 架構規範 | [docs/decisions/0018-realtime-architecture.md](docs/decisions/0018-realtime-architecture.md) |
| Phase 0.1 | [docs/changes/2026-05-08-phase-0.1-complete.md](docs/changes/2026-05-08-phase-0.1-complete.md) |
| Phase 0.2 | [docs/changes/2026-05-08-phase-0.2-complete.md](docs/changes/2026-05-08-phase-0.2-complete.md) |
| Phase 0.3 | [docs/changes/2026-05-08-phase-0.3-complete.md](docs/changes/2026-05-08-phase-0.3-complete.md) |
| Phase 1 | [docs/changes/2026-05-08-phase-1-complete.md](docs/changes/2026-05-08-phase-1-complete.md) |
| Phase 2 | [docs/changes/2026-05-08-phase-2-complete.md](docs/changes/2026-05-08-phase-2-complete.md) |
| Phase 3 | [docs/changes/2026-05-08-phase-3-complete.md](docs/changes/2026-05-08-phase-3-complete.md) |
| Phase 4 | [docs/changes/2026-05-08-phase-4-complete.md](docs/changes/2026-05-08-phase-4-complete.md) |
| e2e 補完 | [docs/changes/2026-05-08-multi-e2e-complete.md](docs/changes/2026-05-08-multi-e2e-complete.md) |
| v2 16 項打磨 | [docs/changes/2026-05-08-admin-multi-sessions-v2.md](docs/changes/2026-05-08-admin-multi-sessions-v2.md) |

### 📋 下一步

> **🎯 接續指南** → [docs/changes/2026-05-09-next-action-guide.md](docs/changes/2026-05-09-next-action-guide.md)
> 下次只要說「繼續」、我會讀此檔接上、並列 A 類可選工作

**短期（業主驗證）**：
1. 業主跑 1-2 場真實多人活動 → 驗證 ws_event_log 真實資料
2. 截圖比較改前 vs 改後（進入遊戲斷線率、猜謎結算一致性）
3. 觀察 Telegram 告警是否誤觸（cooldown / threshold 視情況調）

**可立即接續**（不需業主先做事）：
- **A1**: ws_event_log Dashboard（推薦、1.5-2 天）
- A2: Phase 5 solo/match WS 整合（評估、1.5-2 天）
- A3: 真實 e2e 跑全套（30 分、需重啟 dev server）
- A4: CI workflow 整合 e2e + ADR check（30 分）
- A5: 補充功能完整性 checklist（1-2h）

**1 週後評估**：是否需 PostgreSQL partitioning（看 ws_event_log 表大小）

---

### Phase 5 — 多人元件 L3 全套完成（2026-05-05）
- **基礎建設**：3 張新 DB 表（team_game_states / team_lock_states / team_shooting_hits）
  + 3 個新 server routes + 2 個通用 client hooks（useTeamGameState / useTeamPagePersistence）
- **元件升級**：VoteTeam / ShootingTeam / LockCoop / RelayMission / TerritoryCapture /
  CollectiveScore / RoleAssign / QuestChain / JigsawPuzzle / TreasureHunt / GpsCascade
  全部重整後狀態不遺失、多玩家進度同步
- **Bugs fixed**：VoteTeam server broadcast、TerritoryCapture API 解析、ShootingTeam credentials
- **TypeScript 0 errors，E2E smoke test 51/51 全綠**

---

## 🎮 2026-05-01～02 — 多人遊戲元件框架（Phase 1+2 完整）

**累計 13 個 commits、142 新單元測試、4 個多人元件全鏈路**

### 規劃文件

[docs/GAME_COMPONENT_MULTIPLAYER_PLAN.md](docs/GAME_COMPONENT_MULTIPLAYER_PLAN.md)（v1.3，1057 行）— 長期接力紀錄文件

核心設計原則：
- **元件三分法**（不對稱約束）：shared（4 個通用） + solo（18 個個人專用） + multi（多人專用）
- **playerMode 從 gameMode 推導**：不新增 schema 欄位，依 DRY 原則
- **約束不對稱**：multi 元件嚴格只給多人遊戲；solo 元件兩種模式都可用
- **使用者體驗一致**：玩家介面只看到「隊伍 / 戰隊 / 對講機」，無英文技術詞露出

### Phase 1：地基建立（7 commits）

| Phase | Commit | 內容 |
|-------|--------|------|
| 1.1 | `5db5565e` | 元件分類常數 + 8 helpers（[shared/multiplayer-component-types.ts](shared/multiplayer-component-types.ts)） |
| 1.2 | `68288f1c` | 建立 `solo/` `multi/` `shared/` 目錄骨架 |
| 1.3 | `77e3602e` | `derivePlayerModeFromGameMode` helper（取消 schema 變更） |
| 1.4 | `ca19ad98` | server page 儲存約束 + 不對稱規則修訂 v1.2 |
| 1.5 | `c43208a4` | useGameTimer 共用 hook + 10 測試 |
| 1.6 part 1 | `9f4e3661` | 4 通用元件 → shared/components/ + 1 多人元件 → multi/ |
| 1.6 part 2 | `8a70cd2b` + `d38f17e6` | 18 個個人元件 → solo/ + 修 10 個測試 import 路徑 |

成果：
- 23 個元件分類就位（shared 4 + solo 18 + multi 1）
- git rename 完整保留 history（94-100%）
- 三層驗證：tsc + GamePageRenderer 17/17 + production build

### Phase 2：核心多人元件（11 commits / 115 新測試）

| Phase | Commit | 內容 | 測試 |
|-------|--------|------|------|
| 2.1 | `3ace636d` | GameFormDialog 加 gameMode 選擇器（4 種類型） | - |
| 2.2 | `420ba76f` | TeammatePanel 共用 UI（compact / full 兩版型） | 12 |
| 2.3 part 1 | `3690933a` | VoteTeam 元件 MVP（majority/unanimous/display） | 23 |
| 2.3 part 2 | `a3beb1ed` | useTeamVoteSync hook（API + WebSocket 訊息處理） | 16 |
| 2.3 part 3 | `b22a6ebb` | VoteTeamPage 容器 + GamePageRenderer 註冊 | - |
| 2.4 part 1 | `7ae5293f` | ShootingTeam 元件 MVP（隊伍累計分 + 排行榜） | 25 |
| 2.4 part 2 | `3d358354` | useTeamShootingSync hook + ShootingTeamPage + 註冊 | 12 |
| 2.5 part 1 | `d1037f6b` | GpsTeamMission 元件 MVP（any/all 觸發模式） | 27 |
| 2.5 part 2 | `6a057e28` | GpsTeamMissionPage 容器 + 註冊 | - |

成果：
- 4 個多人元件全鏈路就位（PhotoTeam 已有 + 3 個新建）
- Admin 可在 GameFormDialog 選遊戲類型（個人/隊伍/競技/接力）
- 玩家進多人遊戲：自動取 myTeam → 接 WebSocket → 多人協作 UI

### 已完成的玩家旅程（架構層）

```
Admin 建多人遊戲（GameFormDialog 選「隊伍協作」）
   ↓
玩家進入 → /api/games/:gameId/my-team 取 teamId + members
   ↓
進入 vote_team / shooting_team / gps_team_mission page
   ↓
自動連 WebSocket / 自動取 GPS 位置（依元件類型）
   ↓
達標後 onComplete 前進
```
...
## 🛡️ 2026-05-02 — Phase 2.5 多人遊戲穩定性軸（7 commits / 9 新測試）

實機驗證 Phase 2 期間發現多人遊戲執行時 7 個關鍵 bug，這個軸與元件層 Phase 1-4
**並列**補完穩定性層。雙軸架構在 [docs/GAME_COMPONENT_MULTIPLAYER_PLAN.md](docs/GAME_COMPONENT_MULTIPLAYER_PLAN.md) v1.5 正式定義。

| Phase | Commit | 內容 | DB schema |
|-------|--------|------|-----------|
| 2.5.0 | `f28edd45` | field routing fix（點賈村跑後浦） — Provider Context 共享 themePayload | - |
| 2.5.1 | `37bd7a6b` | 重連 1 秒 flash + 自願退出（離開遊戲呼 leave API 設 leftAt） | - |
| 2.5.2 | `005c6429` | 隊友離線 / 重連 / 離開 toast — 訊息語意拆三類 | - |
| 2.5.3 | `9941ae67` | ghost lobby fix（status='playing' 但無 active session 視為已結束） | - |
| 2.5.4 | `eb69d0f1` | 隊伍最快進度同步（B 簡化版，個人分數各自累計） | ⚠️ `team_sessions.max_page_index` |
| 2.5.5 | `1e7b6101` | 寬限期計時 — 30s grace + 120s auto-leave（純 in-memory） | - |
| 2.5.6 | `fec26059` | TTS 語音通知（瀏覽器 SpeechSynthesis，volume 0.4，60s throttle） | - |

**完整斷線重連流程**：
```
玩家 A 斷線 → broadcast disconnected (toast + 🔊)
  → 0~30s 重連 → cancelTimer + reconnected (toast + 🔊)
  → 30s 沒回 → grace_expired (destructive toast + 🔊)
  → 30~150s 仍可重連
  → 150s 沒回 → DB leftAt + broadcast left (toast + 🔊)
  → 下次開遊戲 → my-team 回 null → 不被自動拉回
```

**設計決策**：
- 進度同步：B 跟最快進度（簡化版，個人分數各自累計）
- 暫停策略：全自動 hybrid（無隊長 dialog，後續加強版可加）
- 寬限期實作：純 in-memory（server 重啟 reset，可接受）
- TTS 語音：瀏覽器內建 SpeechSynthesis（免費 / 中文 zh-TW / 60s throttle）

---

## 📋 2026-05-02 凌晨 — Phase 2.6 對焦完成（2 commits）

**對焦背景**：使用者要求對照 [GAME_COMPONENT_MULTIPLAYER_PLAN.md](docs/GAME_COMPONENT_MULTIPLAYER_PLAN.md)
做完整盤點，發現 Phase 2.5 穩定性軸不在規劃內 → 需要寫進文件。同時驗證 Phase 2.6
列出的 6 個阻塞項實際狀況。

| 項目 | 對焦結果 | 處理 |
|------|---------|------|
| Admin page editor multi pageType | 已就位（PAGE_TYPES + ToolboxSidebar 過濾） | 無需動手 |
| VoteTeamPage WS 接合 | 已就位（line 96-101 訂閱 onMessage） | 無需動手 |
| shooting_hit userId | 已就位（enrichShootingRecordForBroadcast） | 無需動手 |
| WalkieSuggestionToast | 缺 | ✅ 新建 useWalkieSuggestion hook（commit `5b04244b`） |
| TeammatePanel 整合 | 部分（ShootingTeam 已用） | ⏸️ 不強推（VoteTeam/GpsTeam 特化資訊比通用 panel 合適） |
| HMAC 防作弊 | 未做 | ⏸️ 工程量大暫緩（後批處理） |

**Commits**：
- `15cdeada` — docs(plan) v1.5 加 Phase 2.5 穩定性軸（雙軸架構）
- `5b04244b` — feat(team) 乙 4 WalkieSuggestionToast + 3 元件整合 + 對焦驗證紀錄

---

## ✅ 2026-05-02 凌晨 — Phase 2.7（ChoiceVerifyRace 已先行）

對焦時發現 [client/src/components/game/multi/ChoiceVerifyRace.tsx](client/src/components/game/multi/ChoiceVerifyRace.tsx)
+ ChoiceVerifyRacePage.tsx 已實作，但未列入規劃文件 Phase 進度（原計劃在 Phase 3 第 1 週）。
v1.6 修訂時把它從 Phase 3 移出，標為 Phase 2.7（已先行完成）。

PAGE_TYPES 已含 `choice_verify_race`，admin 可建立此元件。

---

---

## 🎮 2026-05-02 — Phase 3+4 完整實作（16 commits / 60 新測試）

承接對焦完成的 v1.6 規劃，用 /loop 2 分鐘節奏推進元件層 Phase 3+4。

### Phase 3.2 LockCoop（協作解鎖 — 5 commits / 20 測試）

| 步驟 | Commit | 內容 |
|------|--------|------|
| 1+2 | `1f5cf133` | schema LockCoopConfig + 純 UI 元件（fnv1aHash + pickClueIndexForUser 線索分配 helper）+ admin PAGE_TYPES |
| 3 | `27e82c69` | server WS team_lock_coop_sync + sendLockCoopSync + useTeamLockCoopSync hook |
| 4 | `4aa4a8f0` | LockCoopPage 容器 + GamePageRenderer 註冊 |
| 5 | `56f3283f` | 20 個單元測試（純函式 + 元件三狀態 + 互動） |

**玩家流程**：admin 拖「協作解鎖 👥」 → 設定密碼 + 多組線索 → 玩家進去自動拿分配的線索（hash 穩定）→ 任一人輸入即同步 → 任一人按嘗試 → 全隊看到結果

### Phase 3.3 RelayMission（接力任務 — 5 commits / 18 測試）

| 步驟 | Commit | 內容 |
|------|--------|------|
| 1+2 | `bbb94e4c` | schema RelayMissionConfig（segments[]）+ 純 UI（pickPlayerForSegment 復用 fnv1aHash）+ PAGE_TYPES |
| 3 | `c3645699` | server WS team_relay_sync + sendRelaySync + useTeamRelaySync hook（正確驗證 + 推進 + 廣播） |
| 4 | `1d355391` | RelayMissionPage 容器 + 註冊 |
| 5 | `5fcc3d48` | 18 個單元測試（含 isSegmentAnswerCorrect / pickPlayerForSegment 公平打散） |

**玩家流程**：admin 拖「接力任務 👥」 → 設 N 段問答 → 玩家進去自動分配段落 → 輪到自己時答題 → 答對廣播給隊友推進到下一段 → 全部完成 Trophy 慶祝

### Phase 3 §7.4 WS Reconnect 狀態恢復（1 commit）

| Commit | 內容 |
|--------|------|
| `d69ccc8e` | server in-memory `teamStateCache` Map 緩存每 team 最新 state；`team_join` 時 snapshot send 給新連線 |

解決問題：LockCoop / RelayMission 純 client 廣播，新進來 client 看不到歷史 → 現在 reconnect 自動拿最新狀態。

### Phase 4 TerritoryCapture（地盤戰 — 5 commits / 22 測試）

| 步驟 | Commit | 內容 |
|------|--------|------|
| 1 | `82718065` | schema TerritoryCaptureConfig（points/timeLimitSec/cooldownSec）+ PAGE_TYPES |
| 2 | `f36ce543` | 純 UI（distanceMeters Haversine + computeRanking + canCapturePoint 5 種情境 helpers） |
| 3 | `5b994f1e` | server WS territory_capture_sync（**broadcastToSession 多隊共享**）+ sendTerritorySync + alsoJoinSessionId（雙 room）+ useTeamTerritorySync hook |
| 4 | `38f7bcb5` | TerritoryCapturePage 容器（GPS + client-side 倒數）+ 註冊 |
| 5 | `4e6e12b8` | 22 個單元測試（純函式 + 元件 + 互動） |

**玩家流程**：admin 設多個 GPS 點 + 時限 → 玩家進去自動 join session room → 接近點顯示佔領按鈕（含 cooldown）→ 廣播給所有隊伍即時看到變化 → 時限到結算贏家

**關鍵設計**：TerritoryCapture 是「session 範圍」廣播（多隊共享），透過 `alsoJoinSessionId` 讓 useTeamWebSocket 同時加入 team + session 兩個 room。

### v1.7 文件更新（1 commit）

`0450bc76` — MULTIPLAYER_PLAN v1.7 + §3.3 / §6.5/§6.7/§6.8 / §11 全部標 ✅
...
| PhotoTeam | 1 | (隊長主控) | - |
| VoteTeam | 2 | team | 39 |
| ShootingTeam | 2 | session | 37 |
| GpsTeamMission | 2 | team | 27 |
| ChoiceVerifyRace | 2.7 | team | (先行) |
| LockCoop | 3.2 | team | 20 |
| RelayMission | 3.3 | team | 18 |
| TerritoryCapture | 4 | **session** | 22 |

### 暫緩項（非阻塞）

🟡 HMAC 防作弊（ShootingMission） — P0 安全項，工程量大
🟡 隊長 dialog（leader-decide UI）
🟡 後台寬限期可調整 UI
🟡 語音開關 UI（玩家可關閉 TTS）
...
🟡 HMAC 防作弊（ShootingMission simulateHit 簽章 + server 驗證） — P0 安全項，工程量大
🟡 隊長 dialog（leader-decide UI） — 寬限期過時讓隊長選「等待 / 先繼續」
🟡 後台寬限期可調整 UI — field settings 加 disconnectGracePeriod / pauseStrategy 欄位
...
|------|------|------|
| v1.0 | 初版 | 規劃完成 |
| v1.1 | 取消 schema 加 playerMode 欄位 | 發現 gameMode enum 已能推導 |
| v1.2 | 約束從對稱改不對稱 | 盤點線上 28 個 team 遊戲全用 solo 元件 |
| v1.3 | Phase 1 完成 + 子目錄保留原位 | 子目錄純內部工具，搬會更複雜 |
| v1.4 | Phase 2 完成 — 4 個多人元件全鏈路 | VoteTeam / ShootingTeam / GpsTeamMission |
| v1.5 | 加 Phase 2.5 穩定性軸（雙軸架構） | 實機驗證發現 7 個穩定性 bug |
| v1.6 | 全面對焦 — Phase 2.6 處理、ChoiceVerifyRace 補認列為 2.7 | 使用者要求對焦 |

### 元件分類常數（單一真實來源）

[shared/multiplayer-component-types.ts](shared/multiplayer-component-types.ts) 提供：
- `SHARED_COMPONENTS` (4)：text_card, dialogue, video, flow_router
- `SOLO_ONLY_COMPONENTS` (18)：button, vote, lock, shooting_mission, photo_*, gps_mission, ...
- `MULTI_ONLY_COMPONENTS` (8)：photo_team, vote_team, shooting_team, gps_team_mission + 規劃中 lock_coop / choice_verify_race / relay_mission / territory_capture
- 8 個 helper 函式（isSharedComponent / isComponentAllowedForGameMode 等）

---

## 🎨 2026-04-30 — 25 輪元件視覺微調 + 場域首頁封面編輯

**累計 25 個 commits、改動 30+ 個元件 + 1 個重大新功能**

### 重大新功能：場域首頁封面編輯（admin 拖拉調焦點）

新增可重用 `EditableCoverImage` 元件，admin 可在前台首頁直接：
- 拖拉調整封面圖焦點位置（CSS object-position）
- 一鍵上傳新封面（不必到後台）
- 應用於：場域 hero banner + 每張遊戲卡片

實作細節：
- `client/src/components/shared/EditableCoverImage.tsx`（380 行新元件）
- `client/src/lib/cover-position.ts` 純函式 util + 21 個單元測試
- Schema：`fields.theme.coverImagePosition` + `games.cover_image_position`
- Zod 限制格式 `"X% Y%"` 防 CSS 注入
- 4 個 E2E 契約測試鎖定後端 API

### 4 次 bug fixes（hero banner 儲存）

`5c802670` → `dd639599` → `f7b56f61` → `03d89a19`：
1. fetchWithAdminAuth 誤用（雙重 .json()）
2. GET endpoint 漏 coverImagePosition + Cache-Control 太久
3. invalidate predicate 不匹配 Provider query key
4. **PATCH 端點打錯**（應為 `/settings` 不是 `/:id`）— 真根因

### 23 個元件視覺微調（輪 7-23）

統一加入：
- `active:scale-[0.92~0.99]` 觸感回饋
- `tabular-nums` 防數字位數變化抖動
- `transition-colors` / `transition-all` 平滑過渡
- 倒數兩段警示分級（≤30s 黃 / ≤10s 紅閃）

涵蓋元件：
| 類別 | 元件 |
|------|------|
| 文字/對話 | TextCard、Dialogue（+ portrait 大圖模式） |
| 驗證類 | TextVerify、ChoiceVerify、ConditionalVerify、Vote、Button |
| Photo（8） | Mission、Spot、Compare、BeforeAfter、Burst、Ar、Team、Ocr |
| 硬體類 | Lock、TimeBomb、MotionChallenge、QrScan、Shooting |
| 媒體類 | Video、FlowRouter |
| GPS | GpsMission、GpsAccuracyIndicator、MotionPermissionRequest、PageLocationMiniMap |

### 測試覆蓋

- 21 個 cover-position 單元測試（NaN/Infinity/clamp/round-trip）
- 4 個 E2E 契約測試（API 結構、Cache-Control、PATCH 端點、CSS 注入防護）
- lib/ 全部 370 個測試保持綠

### 待部署清單（自上次部署 `03d89a19` 後）

- `2349fcc3` cover-position util + tests
- `8e928032` E2E 契約測試

---

## 🛡️ 2026-04-26 深夜 v2 — 5 個 HIGH 安全 issue 收尾

### 修復清單（commit e7cfce3d）

| # | 端點/檔案 | 風險 | 修復 |
|---|----------|------|------|
| 1 | `location-tracking POST /visit` | 玩家不在 session 也能打卡刷分 | 強制 progressList 驗證 + location.gameId 比對 |
| 2 | `location-tracking GET /location-history /visits` | 任意讀別人軌跡 | 限定 = req.user 或同隊隊友 |
| 3 | `auth POST /platform-owner-login` | secret 比對非 timing-safe + 無 brute-force 防護 | `crypto.timingSafeEqual` + per-IP 5 次 / 30 分鐘鎖定 |
| 4 | `webhook-recur` 記憶體 Set 冪等 | 多 container 部署時失效，重複授權 | 改用 PostgreSQL `webhook_processed_events` 表 + INSERT ON CONFLICT |
| 5 | per-user rate limit 缺失 | score / visit / cloudinary-upload 無頻率限制 | team-scores、matches/score、location-visit 加 hotPathLimiter；media/cloudinary/upload 加 uploadLimiter |

### 關鍵實作

**timing-safe 比對 helper**：
```typescript
function timingSafeStringEqual(a: string, b: string): boolean {
  // 用 Buffer + crypto.timingSafeEqual，長度不同也跑一次 dummy 比對
}
```

**webhook 持久化冪等性**（取代記憶體 Set）：
```sql
INSERT INTO webhook_processed_events (event_id, source)
VALUES ($1, 'recur')
ON CONFLICT (event_id) DO NOTHING
RETURNING event_id  -- 0 row = 已處理過
```
- 自動建表（idempotent CREATE TABLE IF NOT EXISTS）
- 自動清理 30 天前舊紀錄
- DB 失敗 fail-open（不阻斷付款流程）

**per-IP brute-force 防護**：
- 5 次失敗 → 30 分鐘鎖定
- 記憶體 Map（多 container 各自計數，可接受）
- 每分鐘自動清理過期計數

---

## 🛡️ 2026-04-26 深夜 — 11 個 Critical 跨場域漏洞修復（正式開放前安全強化）

### 整體進度（截至 commit 20056c99）

| 階段 | commit | 內容 |
|------|--------|------|
| AI 修復 | `e530d87b` | 4 個 AI Bug + Dashboard 跨 provider 統計 |
| Critical 1 批 | `33d4654c` | admin-rewards / admin-items / sessions/cleanup / scheduler lock |
| 防作弊 | `1311ee13` | 每日上限 10 場 |
| Critical 2 批 | `20056c99` | 玩家作弊 + 9 個跨場域 IDOR |

### 本批修復清單（commit 20056c99）

**新增 `server/lib/field-ownership.ts`**：
- `assertFieldOwnership(admin, fieldId, res)` — 一行通用檢查
- super_admin / platform_admin 通過所有檢查
- field_admin 強制只能存取自己場域

**11 個 Critical 跨場域 / 作弊漏洞**：

| # | 端點 | 風險 | 修復 |
|---|------|------|------|
| 1 | `team-scores POST /:teamId/score` | **作弊任意改分**（無 membership / delta / sourceType 限制） | teamMembers JOIN + delta [-1000,1000] + sourceType 白名單 |
| 2 | `matches PATCH /:matchId/score` | **玩家自評最高分**（任意整數） | score [0,100000] + delta ≤ 500 + 禁倒退 |
| 3 | `admin-engagement` 7 端點 | 跨場域改設定 / 通知劫持 | 全加 fieldId 隔離 |
| 4 | `admin-purchases` 3 端點 | 跨場域授權 / 退款 | game.fieldId 比對 |
| 5 | `admin-redeem-codes` 6 端點 | 跨場域兌換碼操控 | redeemCode.fieldId 比對 |
| 6 | `admin-chapters` 6 端點 | 跨場域章節 CRUD | game.fieldId / chapter.gameId 雙重 |
| 7 | `admin-battle PATCH /rankings/:id` | 跨場域改 rating | ranking.fieldId 比對 |
| 8 | `admin-walkie POST /broadcast-tokens` | 跨場域對講機廣播 | JOIN games 過濾 + selected sessionIds 驗證 |

### 涉及檔案統計

- 新增：`server/lib/field-ownership.ts`
- 修改：8 個 server route 檔案

### 部署驗證

- commit `20056c99` 已上線 game.homi.cc
- TypeScript 編譯零錯誤
- `/api/version` 回傳新 commit ✓

### 待後續處理（非 Critical）

- HIGH: location-tracking 玩家作弊 + 隱私洩漏
- HIGH: webhook-recur 記憶體 Set 改 DB unique constraint（多 container 安全）
- HIGH: per-user rate limit（score / visit / handoff / cloudinary-upload）
- MEDIUM: PLATFORM_OWNER_SECRET 改 timingSafeEqual

---

## 🔒 2026-04-26 晚 — 4 個 Critical 安全漏洞修復（正式開放前）

### 修復清單（commit 33d4654c）

| # | Issue | 風險 | 修復 |
|---|-------|------|------|
| #4 | `admin/rewards/manual` cross-tenant | 場域 admin 可發券給其他場域 | `squad.homeFieldId` 比對 + audit trail |
| #6 | `admin/items/:id` GET/PATCH/DELETE cross-tenant | 場域 admin 可看/改/刪其他場域 item | 新增 `checkItemFieldOwnership` helper |
| #8 | `admin/sessions/cleanup` cross-tenant | 場域 admin 可清其他場域 sessions | `abandonStaleSessions(threshold, fieldId)` 過濾 |
| C3 | scheduler 重複執行 | 多 container 水平擴展時各自執行 | 用 `pg_try_advisory_lock` cluster lock 包裹 4 個 scheduler |

### 實作要點

**`server/lib/cluster-lock.ts`**（新檔）
- 用 PostgreSQL advisory lock（cross-connection 唯一）
- `withClusterLock(key, fn)` — 取不到鎖時跳過、不阻塞
- 適用於：lifecycle / achievement / dormancy / monthly-reset 4 個 scheduler

**fieldId 隔離原則統一**
- super_admin / platform_admin：不限制（可跨場域）
- field admin：強制只能存取自己場域資源
- audit trail：手動發券記錄 issuedByAdminId + issuedByFieldId

### 部署驗證

- commit `33d4654c` 已上線 game.homi.cc
- 4 schedulers 啟動 log 正常
- TypeScript 編譯零錯誤
- `/api/version` 回傳新 commit ✓

---

## 🤖 2026-04-26 — AI 用量紀錄系統修復（用戶反映「AI 使用紀錄都是 0」）

### 問題盤點（4 個關鍵 bug）

| # | 問題 | 原因 | 影響 |
|---|------|------|------|
| 1 | ai_usage_logs 全 0 | `ai-scoring.ts` 3 個 Gemini 端點完全沒寫 log | 「AI 使用紀錄都是 0」 |
| 2 | 本地 DB 缺表 | `ai_usage_logs` 表從未在本地 push | 本地無法測試 logger |
| 3 | OCR 檔案掛不到容器 | `docker-compose.prod.yml` 沒 mount `./secrets:/app/secrets:ro` | OCR 全部 ENOENT |
| 4 | 場域 enableAI 是 NULL | 賈村場域 settings 沒明確設 `enableAI: true` | 不影響運作但語意不明 |

### 解決方案

**1. 新增 `server/lib/ai-usage-logger.ts`**：通用 logAiUsage 函式
- 支援 4 個 provider：`gemini`/`openrouter`/`google-vision`/`mediapipe`
- 失敗不阻斷主流程（try/catch 吞）
- 結構化 metadata（不存敏感資訊）

**2. 重構 `server/routes/ai-scoring.ts`**：3 個 Gemini 端點全部加上 log
- `verify-photo`：成功 + 6 種失敗狀態（AI_NOT_CONFIGURED / RATE_LIMITED / FIELD_AI_DISABLED / QUOTA_EXCEEDED / API_KEY_INVALID / NETWORK_ERROR）
- `compare-photos`：同上
- `score-text`：同上
- 引入 `resolveAiContext`：同時取得 apiKey + fieldId（給 logger 用 fieldId 做場域聚合分析）
- 自動推測 errorCode（從 error message 用 regex 判斷）

**3. 修補 `docker-compose.prod.yml`**：app service 加上 `volumes: ./secrets:/app/secrets:ro`

**4. 場域 enableAI 明確設為 true**：本地 + 生產 DB UPDATE

**5. 生產權限修正**：`chmod 755 secrets/ && chmod 644 gcp-vision.json`（讓容器內 node user 能讀）

### 部署結果

- commit `c9e028b3` push 並重 build
- 生產 `/api/version` 確認 commit ✓
- node user 可讀 `/app/secrets/gcp-vision.json` ✓
- ai_usage_logs 表正常工作（已記錄 4 筆 google-vision 失敗 log）

### 已知次要問題

- **GCP 帳號 billing 未開**：OCR 會記到 `PERMISSION_DENIED`，需到 [GCP Console](https://console.developers.google.com/billing/enable?project=885341500294) 啟用帳單
- **後浦金城 場域**：尚未設 Gemini API key，需在管理後台設定

### 預期效果

部署後：
- 玩家觸發 AI 照片驗證 / 文字評分 / 雙圖比對 → 自動寫入 ai_usage_logs
- 管理端 `/admin/ai-usage` 頁面可看到完整統計（成功率 / 平均延遲 / 錯誤分布）
- 跨場域用量分析可用 `field_id` 做 GROUP BY

---

## 🚀 2026-04-25 晚上 — Squad System Phase 12 完整上線 + 5 關鍵 bug 修復

### Phase 12 全套（已部署）

依 `docs/SQUAD_SYSTEM_DESIGN.md` 完成 Phase 12 全套 7 大功能 + 1 個加值 dashboard：

- **12.1 推廣連結追蹤**（§13.4 §20.7）— `squad_invites` schema + 6 端點 + 接受頁
- **12.2 6 個排行榜**（§8）— 場次榜 / 名人堂 / 新人榜 / 上升星 / **常客榜** / 段位榜
- **12.3 玩家端歡迎隊伍 Dialog**（§14）— 第一次進場域延遲顯示，30 天記憶
- **12.4 自動成就 cron**（§9.2 §9.3 §9.5）— 16 個成就 + 每 6 小時 cron
- **12.5 隊名改名冷卻**（§17.3 §17.4）— 30 天冷卻 + 解散 180 天鎖名
- **12.6 場域 admin 行銷設定 UI**（§13 §14 §16 §18）— 5 tab
- **加值 獎勵分析 Dashboard**（§26.7 §26.8）— KPI + 兌換率 + Top 10
- **Backfill 腳本** — `scripts/backfill-squad-records.ts`

### 完整審查發現的 5 個關鍵 bug（已修復本地，待部署）

依詳細 audit 後發現 Phase 12 雖然 schema/UI 上線，但 **核心數據鏈路有 5 個 bug 影響正確性**：

| # | Bug | 修復前 | 修復後 |
|---|---|---|---|
| 1 | `writeSquadRecordFromSession` PvE rating 永遠 0 | 寫死 `ratingChange: 0` | 走完整 `calcRewards`，含 ELO + bonus + crossField |
| 2 | `battle-results.ts` squadId 用 `player:${userId}` | `fieldId: slot.venueId`（venueId 而非 fieldId） | 真實 `clan.id` + `venue.fieldId`，clan 玩家共用 squadId |
| 3 | aihomi webhook 永遠不送 | `stageExternalReward` 只 stage 進 DB | fire-and-forget 呼叫 `sendAihomiReward` |
| 4 | `exp_points` 規則沒寫值 | 註解說「已處理」實際沒加 | 新增 `issueExpPoints` 直接更新 squadStats |
| 5 | 24h 同對手限制邏輯錯誤 | 看「該隊 24h 任意 5 場」 | 用 `performance->>opponentSquadId` 精準比對 |

### 真實完成度（對 SQUAD_SYSTEM_DESIGN.md）：約 **65%**（修 bug 後）

| 區塊 | 完成度 | 備註 |
|------|------|------|
| Schema / 基礎建設 | 85% | 缺 `squads` 主表、`squad_members`（暫用 battle_clans）|
| 核心計分（rating + 5 模式 + ELO + bonus）| 85% | bug 修復後 PvE 也算分；缺名次型計算（接力 8 隊）+ 少數 bonus |
| 排行榜 + 徽章 | 80% | 缺 monthlyGames 月度重置 cron、tier 升級自動化、特殊徽章 |
| 推廣（招募 + 邀請 + dashboard）| 75% | 招募雙向獎勵實寫已修；缺超級隊長段位自動寫入 |
| 留存通知 | 70% | dispatcher 完整；email 仍 console.log；cooldown 預設與設計不對齊 |
| 獎勵轉換節點 | 80% | 規則引擎 + aihomi webhook + exp_points 已修；缺 perDay 配額、retry worker |
| UX 整合 | 35% | 統一參戰 Dialog 元件做了沒接、玩家自評 UI 沒做、BattleClan→/squad/:id redirect 沒做 |

### 還剩下的工作（按業務影響排序）

#### 🔴 Phase 14 阻塞性
1. 新建 `squads` 主表 + `squad_members` 主表 + 資料遷移腳本（§22.3 階段 3）
2. 隊伍 lifecycle 自動轉換（active → veteran → legend）+ tier_upgrade 通知
3. 超級隊長段位自動寫入 `squadStats.superLeaderTier`（cron）
4. 招募獎勵實際寫入 squadStats.totalExpPoints（首戰 +30/+50、5 場達標 +100、雙向 50/100 點）
5. Mode B 玩家自評水彈對戰（雙隊長按勝負 + 一致採信 + 不一致裁判仲裁）

#### 🟡 Phase 15 業務功能
6. 每日上限 10 場 / 重複對戰 4-5 場 50% 計分（§19.3）
7. monthlyGames / monthlyRecruits 月度重置 cron
8. 統一參戰 Dialog 接到實際遊戲入口
9. BattleClan → /squad/:id redirect

#### 🟢 加值
10. 特殊活動徽章（派對達人/親子/嘉年華王者/破紀錄王/速通達人）
11. 名次型計算（接力 8 隊 rank → actual 線性插值）
12. 用時 < 平均 50% / 全員存活 表現加成
13. SEO og:image 自動生成 + Schema.org SportsTeam
14. Email 真接 Resend
15. 通知頻率 cooldown 對齊設計（每 3 天最多 1 則 push、每週 1 封 email）
16. 規則引擎 perDay 配額 + 失敗 retry worker
17. 紀念照集錦（公開分享頁）
18. 隊伍 is_public 公開/私密切換

### 測試成果

- 服務層 165 / 165 單元測試通過（Phase 12 新增 60 個 + bug 修復後保持綠燈）
- TypeScript 零錯誤
- Frontend build 通過

### Git commit 紀錄

- `a9168667` Phase 12 完整推廣 / 排行榜 / 成就 / 設定全套
- `5483e5df` 獎勵分析 Dashboard
- `d8d28cef` squad_stats 重建 backfill 腳本
- `[待 commit]` 5 個關鍵 bug 修復

---

## 🎯 2026-04-26 凌晨 — 對戰中心 22 輪 UX 大升級

### 範圍
玩家端 Battle / 組隊大廳 / Match Lobby 全面優化（20+ 個檔案重構）

### 重點改善（按輪次）

| 輪 | 主題 | 重點 |
|---|------|------|
| 1 | BattleSlotDetail UX | skillLevel 中文化 + 取消報名 AlertDialog |
| 2 | 組隊分享 + BattleHome 高亮 | Web Share API + 下一場 animate-pulse Badge |
| 3 | Battle 5 頁面 raw fetch → apiRequest | 程式碼少 70%，auth 一致 |
| 4 | battle-labels.tsx + 16 tests | slotStatusBadge / skillLevelLabel / TIER_BG/BADGE 抽共用 |
| 5 | battle-time.ts + 18 tests | formatTimeUntil + isImminentSlot + BattleSlotDetail 距離時間 |
| 6 | 隊伍大廳 deep link | `?code=ABC123` 自動帶入 + parseInviteCode 10 tests |
| 7 | 6 頁面剩餘 fetch + 3 個 Dialog | Loader 動畫 + 防中斷 onOpenChange |
| 8 | BattleAchievements + 進度色階 | 雙進度條 + 排序優先 + Trophy/Lock icon |
| 9 | MatchLobby WaitingView 完整邀請 | 複製/分享 + 參賽者列表 + 房主 Crown + 5 tests |
| 10 | FlowRouter 測試強化 | 20 → 33 tests（variable_gt/lt + pickRandom + resolve）|
| 11 | processOnCompleteActions 25 tests | 不可變性 + 多動作組合 |
| 12 | BattleClanDetail 離開確認 + authFetch | Battle 4 頁面 5 處 authFetch → apiRequest |
| 13 | BattleNotifications 視覺強化 + formatTimeAgo | 左邊色塊 + animate-pulse 圓點 + sticky banner |
| 14 | BattleResult 分享 + K/D + 3 CTA | Web Share 戰績 + K/D ratio + 再戰一場 |
| 15 | ClanManagePanel 踢出確認 + 7 處 fetch | apiRequest + AlertDialog 防誤觸 |
| 16 | 全專案 authFetch sweep | 玩家端 raw fetch + authFetch 殘留：0 |
| 17 | BattleHistory 統計總覽 + Empty CTA | 6 欄統計 + 累積積分變動 + Swords CTA |
| 18 | BattleRanking 排名追求動力 | 距離下一名提示 + 定位到我 scrollIntoView |
| 19 | BattleSeasonHistory 賽季總覽 + CTA | 最佳名次/最高積分/最高段位/總勝率 + tierOrder helper |
| 20 | Empty state CTA 全面化 | BattleNotifications + BattleMyProfile 雙 CTA |
| 21 | queryClient.test.ts 友善訊息修正 | 401 → 「登入已失效，請重新登入」對應測試 |
| 22 | VideoPage testid + OptimizedImage retry | 修 6 個 pre-existing 失敗測試 |

### 新建共用 lib

- **`client/src/lib/battle-labels.tsx`** (97 行)
  - `SLOT_STATUS_INFO` + `slotStatusBadge()` JSX helper
  - `SKILL_LEVEL_LABEL` + `skillLevelLabel()`（含 expert→advanced alias）
  - `TIER_BG` (卡片用) + `TIER_BADGE` (badge 用) + `tierBgClass()` + `tierBadgeClass()`

- **`client/src/lib/battle-time.ts`** (140 行)
  - `formatTimeUntil()` — 「⚡ 5 分鐘後 / 今天 14:00 / 3 天後 19:00 / 對戰中 / 已結束」
  - `isImminentSlot()` — 30 分鐘內判斷
  - `formatTimeAgo()` — 「剛剛 / 5 分鐘前 / 3 天前 / 1 年前」

### 新增單元測試（107 個）

| 檔案 | tests | 內容 |
|-----|-------|------|
| `battle-labels.test.ts` | 16 | slot status / skill level / tier 顏色 |
| `battle-time.test.ts` | 28 | formatTimeUntil + isImminentSlot + formatTimeAgo（注入 now）|
| `parseInviteCode.test.ts` | 10 | URL 解析 + 防注入（特殊字元/長度限制）|
| `flow-router.test.ts` | +13 | variable_gt/lt/gte/lte + pickRandom + resolveFlowRouter |
| `flow-router.test.ts` | +25 | processOnCompleteActions（set/inc/dec/toggle/add_item/remove/score）|
| `MatchViews.test.tsx` | +5 | 複製/分享/參賽者列表/「我」 Badge/等待提示 |
| 既有測試修復 | 6 | queryClient 401 + VideoPage testid + OptimizedImage retry |
...
|-----|------|
| 重構檔案 | 20+ 個玩家端頁面/元件 |
| 玩家端 raw fetch 殘留 | **0**（從 11+ 處 → 0）|
| 玩家端 authFetch 殘留 | **0**（從 7+ 處 → 0）|
| Dialog 防誤觸 | **3 個**（取消報名 + 離開戰隊 + 踢出成員）|
| Empty state CTA | **5 個**（History + SeasonHistory + Notifications + MyProfile.戰隊 + ...）|
| 統計總覽 | **3 個**（History + Achievements + SeasonHistory）|
| 排名動力提示 | 1 個（BattleRanking 距離下一名）|

### 體驗一致性達成

所有 Battle 玩家端歷史/紀錄類頁面都有：
- 統計總覽卡（在列表頂部）
- 顏色一致（bg-primary/5 border-primary/30）
- Empty state CTA（讓使用者知道下一步要做什麼）
- 統一 apiRequest（friendly error + Firebase token + credentials）

### 測試覆蓋

- **107 個新增 unit tests** — 全部通過（209/209 我相關測試）
- **6 個 pre-existing 測試修復**
- 全 client 從 596 → 600 通過

---

## 🎨 2026-04-25 深夜 — 13 元件 UX 統一優化

### 修復問題

| 問題 | 根因 | 修復 |
|---|---|---|
| AR 面具看不見 | CSP 擋 MediaPipe CDN | connect-src 加 storage.googleapis.com + cdn.jsdelivr.net |
| 切換鏡頭跳掉 | setMode 觸發 video re-mount | switchCamera 不動 mode |
| 連拍 500 上傳失敗 | public_id 同秒碰撞 | 加 random suffix |
| 合成卡 50% 無反饋 | Multi API hang 無 timeout | 15s timeout + fallback + 跳過鈕 + 秒數計時 |
| 下載/分享失敗 | fetch blob 遇 CORS | \<a download\> + shareUrl URL-only |
| 成功頁歪斜 | h-full flex 未置中 | max-w-md mx-auto overflow-y-auto |
| 單頁 crash 白屏 | 沒 ErrorBoundary | GamePageErrorBoundary + 自動上報 |
| 連拍直接開拍沒引導 | stage='shooting' 立即啟動 | 加 preview + countdown 3-2-1 stage |
| 連拍上傳慢 10-15s | sequential 上傳 | Promise.all **並行 5 倍速** |

### 新增 5 個共用元件

1. `lib/share-photo.ts` — shareUrl + triggerDownload util（不 fetch blob）
2. `photo-mission/PhotoSuccessView.tsx` — 統一 7 flow 成功頁（消除 ~350 行重複）
3. `photo-mission/GameLoadingView.tsx` — 全螢幕 loading（含進度條 + 預估時間）
4. `photo-mission/GameErrorView.tsx` — 統一錯誤畫面（Retry + Skip）
5. `GamePageErrorBoundary.tsx` — 全局 crash 保護（影響全部 13 元件）

### 受益元件（13 個）
影像類（8）：photo_mission / photo_spot / photo_compare / photo_before_after / photo_burst / photo_ar / photo_team / photo_ocr
辨識類（4）：text_verify / choice_verify / conditional_verify / qr_scan
影片（1）：video

### 額外功能
- `defaultFacingMode` config — 管理員可設每關預設前/後鏡頭
- 使用者仍可在相機畫面右上 🔄 切換
- AR 預設 user、OCR/Spot/Compare 預設 environment

---

## 🎭 2026-04-25 凌晨 — 三軌道 14 輪完成（OCR + AR + 示範模板）

### Track A (OCR 招牌任務，5/5 輪)

- **A1** `@google-cloud/vision` 整合 + `ai_usage_logs` table + `detectText()`
- **A2** `POST /api/ai/ocr-detect` + fuzzy matching（Levenshtein + 正規化）+ rate limit + 95% auto-fallback
- **A3** `PhotoOcrFlow.tsx` + 新 pageType `photo_ocr` + `PhotoOcrSubConfig`
- **A4** 編輯器 photo_ocr case + 目標文字 + fuzzy threshold slider + 成本提醒
- **A5** `AiUsageCard` 儀表板 + 80% email alert + 95% fallback

### Track B (AR 臉部追蹤，4/4 輪)

- **B1** `@mediapipe/tasks-vision` + `face-landmarker.ts`（6 錨點、合規：純瀏覽器）
- **B2** `PhotoArStickerFlow` 擴充 face anchor loop（RAF + canvas 合成含旋轉）
- **B3** 編輯器 anchorPoint UI（none/face/face_top/eyes/nose/mouth）
- **B4** 效能優化（frame skip 30→15fps）+ 隱私 opt-in Dialog + localStorage 記憶

### Track C (賈村示範模板，3/3 輪)

- **C1** `docs/DEMO_GAME_JIACHUN.json`（8 頁：文字/對話/OCR/QR/GPS/AR/驗證/完賽）
- **C2** `POST /api/admin/games/create-from-demo`（esbuild inline JSON）+ AdminGames 按鈕
- **C3** 實際建立 — **玩家端 URL**: https://game.homi.cc/g/jiachun-demo-4541fc

### 產出

**新增檔案**：`shared/schema/ai-usage-logs.ts` / `server/lib/google-vision.ts` / `client/src/lib/face-landmarker.ts` / `client/src/components/game/PhotoOcrFlow.tsx` / `docs/DEMO_GAME_JIACHUN.json`

**依賴新增**：`@google-cloud/vision@^5.3.5`、`@mediapipe/tasks-vision@^0.10.34`

**DB 變更**：新建 `ai_usage_logs` table（provider/endpoint/success/errorCode/latency + gameId/fieldId/userId + 3 索引 + 3 FK）

**賈村示範遊戲**：id `b3bc7820-db69-4ac3-8fcd-ad1aaf6b6eaa`、slug `jiachun-demo-4541fc`、8 頁、published

---

## 📸 2026-04-24 晚間~深夜 — 相機功能 30 輪迭代（6 種拍照 + 相簿 + 真 GIF + 用量儀表板 + 功能地圖）
...
連續 2 輪失敗即停求決策。最終 **30 輪連續部署、0 失敗、0 回滾**（含 29 輪程式碼 + 2 輪文件）。
