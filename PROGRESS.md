...
> **品牌**：CHITO（台語「七逃」tshit-thô · 走踏、玩耍、閒遊）
> **定位**：走踏在地的實境遊戲 SaaS 平台，多場域獨立營運
> **首發場域**：賈村競技場（JIACHUN）· 後浦金城（HPSPACE）

## 專案概述

CHITO 七逃是一個實境遊戲 SaaS 平台，結合 QR Code 掃描、GPS 定位、團隊合作等互動機制，提供沉浸式戶外遊戲體驗。平台支援多場域獨立營運，每個場域有專屬網址 `/f/{FIELD_CODE}`、獨立主題色系、獨立遊戲內容、獨立排行榜。
