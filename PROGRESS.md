...
**最後更新**: 2026-05-02 凌晨 — Phase 1+2 多人遊戲元件框架完成（13 commits / 142 新測試）
**分支**: main
**Git 狀態**: commit `6a057e28` 已 push 到 main
...
🟡 **Admin page editor** 整合（讓 admin 能在 page editor 選 vote_team 等 multi pageType）
- 目前阻塞項：admin 無法建立含 multi 元件的 page

🟡 **Server side 補完**：
- VoteTeamPage 的 handleWsMessage 還沒接到 useTeamWebSocket（隊友投票無法即時看到）
- shooting_hit 訊息沒附 userId（無法區分誰打的）
- HMAC 防作弊（前端 simulateHit 帶簽章，server 驗證）

🟢 **Phase 3** 新類型多人元件：LockCoop / ChoiceVerifyRace / RelayMission
