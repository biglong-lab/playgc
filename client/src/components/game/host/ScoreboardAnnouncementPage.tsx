...
export default function ScoreboardAnnouncementPage({ page }: ScoreboardAnnouncementPageProps) {
  const rawConfig = (page.config as { config?: ScoreboardAnnouncementConfig } | ScoreboardAnnouncementConfig | null) ?? null;
  const config: ScoreboardAnnouncementConfig =
    (rawConfig && "config" in rawConfig ? rawConfig.config : (rawConfig as ScoreboardAnnouncementConfig | null)) ?? {};
