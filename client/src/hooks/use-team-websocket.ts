...
// ====================================================================
// 🌐 Provider 版（Phase 1 — feature flag = true 時使用）
// ====================================================================
// 透過 WebSocketProvider 共用全域單一 ws connection
// 行為與 Legacy 等價（callbacks 全保留、send functions 全保留）
// 差別：
//   - 不自己 new WebSocket（從 Provider 拿）
//   - 元件 unmount 時不關 ws（Provider 一直保留）
//   - reconnect / keepalive / visibilitychange 由 Provider 統一管
function useTeamWebSocketViaProvider({
  teamId,
  userId,
  userName,
  alsoJoinSessionId,
