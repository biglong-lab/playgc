...
import {
  ...
  CheckSquare, SlidersHorizontal, Network,
  Pencil, Link2,
} from "lucide-react";
...
export const PAGE_TYPES = [
  ...
  // 📺 大螢幕主控元件（原 17 個 host_*）已於 2026-09-25 整條移交 PhotoGo（ADR-0029），編輯器不再提供
] as const;
...
// 🆕 D3 (2026-05-07) — 元件 category 分組
// 把 page_type 依「功能類型」分 5 大類，admin editor 側邊欄按組顯示
// 2026-09-25：原第 6 類「📺 大螢幕主控」整條移交 PhotoGo（ADR-0029），分類一併移除
...
// activity 路線 II/III：narrative + interactive（51 個）
...
//   - 模糊地帶會回到「混在一起」的混亂
//
// 2026-09-25：activity 模式的大螢幕元件（📺 大螢幕主控分類）已移交 PhotoGo（ADR-0029），
//   'activity' 型別保留（games.editor_mode 既有資料仍是這個值），但只剩敘事 + 活動互動兩類。
...
export const EDITOR_MODE_VISIBLE_CATEGORIES: Record<EditorMode, PageCategory[]> = {
  ...
  activity: ["narrative", "interactive"],
};
...
export const EDITOR_MODE_INFO: Record<EditorMode, { label: string; emoji: string; description: string }> = {
  ...
  activity: {
    label: "活動",
    emoji: "🎉",
    description: "活動現場互動（玩家匿名掃 QR、不登入）；大螢幕互動改由 PhotoGo 提供",
  },
};
