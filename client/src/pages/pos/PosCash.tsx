...
export default function PosCash() {
  ...
  // 預設清點型別：未開班→開班；已開班未收班→收班（畫面操作維持原樣；重複由後端擋）
  ...
  return (
    <PosLayout title="櫃檯現金" backTo="/pos">
      ...
      {/* 清帳（僅 cash admin）*/}
      {/* 💸 支出已移至「收支」頁（收款頁切換支出）。此處今日概況仍顯示今日支出合計 */}
      {!locked && (today?.todayExpensesCents ?? 0) >= 0 && (today?.stage === "open" || today?.stage === "closing_done") && (
        <div className="rounded-xl border bg-white dark:bg-slate-900 p-3 mb-3 text-sm flex items-center justify-between">
          <span className="text-muted-foreground">💸 現金支出記在「收支」頁</span>
          <Button size="sm" variant="outline" onClick={() => navigate("/pos/checkout?return=/pos/cash")} data-testid="cash-goto-expense">前往收支</Button>
        </div>
      )}
