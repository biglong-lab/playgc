...
**部署**：`583ef5d9`（bundle `index-DlYSFx1p.js`）；部署前備份 `/opt/backups/db-gamehomicc-predeploy-20260924-1601.sql.gz`

- 業主回報「活動卡片只顯示 NT$249，夜間其實 NT$349」，查生產資料發現的是**漏收錢**：
  時段規則的 `pricePerSlotCentsOverride` 只存在於後台 UI 與型別，
  **後端計價完全沒讀它** → 玩家預約夜間 19:00 場實際只被算 NT$249，每人少收 100 元；
  現場人員電話登記（人工預約）同樣只用活動基價，也少收
- 修正：排程解析讓每一梯帶自己的價格 → 建立預約 / 人工登記都以該梯次計費
  （規則 > 活動基價 > 場域預設）；availability API 回每梯價格；
  預約頁「應付金額」改用選中梯次的價格（原本顯示基價 × 人數，與實收對不起來）
- 顯示：活動卡片（後台 + 前台）顯示區間「NT$249 - NT$349」並加註「依時段不同價」；
  同一天有不同價時，時段按鈕逐格標價；全天同價維持單一價顯示
- 生產驗證：availability 每梯價格正確（日間 24900 / 夜間 34900，夜間活動平日規則未設價 →
  正確回退基價 34900）；業主已把「日間定點射擊」內的夜間規則停用改走獨立活動，
  卡片顯示單一價為正確結果（停用規則不計入）；health ok、5xx = 0
...
- **封面上傳看起來失敗、實際成功，按儲存還會被蓋掉**：各端點回傳欄位名不一致
  （場域 url / 遊戲 coverImageUrl / 活動 coverUrl），元件只認前兩種 →
  活動封面顯示「伺服器沒有回傳 URL」，使用者再按儲存就把剛存好的圖清掉。
  修：元件四種欄位都認、活動端點補回 url
- **活動預約與場域預設徹底解耦**（業主定調「活動就依活動，不要耦合」）：
  顯示時段讀活動排程、建立預約卻驗場域預設 → 新活動必定「該時段不在開放時段內」；
  場域預設被清空後連既有活動也全部預約失敗。
  現在活動只看自己的時段、人數與取消政策，名額依活動分開計算；
  場域層只保留「整場關閉預約」這一個總開關

### 📅 同一天的多段時段（hotfix）

**部署**：`75aa279b`（bundle `index-C_G0I45j.js`）；部署前備份 `/opt/backups/db-gamehomicc-predeploy-20260924-104406.sql.gz`

- 後台設「日間 14:30-17:30」＋「夜間 19:00-20:00」，前台只顯示夜間 →
  同一天只挑一條規則、後加的蓋掉先前的。改成同層規則取聯集（依時間排序、重疊梯次去重）
- 生產驗證：水彈對戰週六由 3 梯（15:00 起）變 5 梯（13:00 起）；
  日間定點射擊週六 15 梯（10:30–17:30）；夜間定點射擊中秋套用假日時段 19:00–20:30

---

## 2026-09-23

### 🏁 競賽 / 接力可用 + 營運 bug 批次（feat + fix）

**部署**：`22dff12c`（bundle `index-Dc7f3Lxc.js`）；部署前備份 `/opt/backups/db-gamehomicc-predeploy-20260923-013325.sql.gz`
詳見 [changes/2026-09-23-platform-overhaul-plan.md](changes/2026-09-23-platform-overhaul-plan.md)（P0 / P1）與
[changes/2026-09-23-identity-rules-match-modes.md](changes/2026-09-23-identity-rules-match-modes.md)

- **競賽 / 接力真的能玩**：伺服器控制倒數 / 開賽 / 時間到 / 全員完成自動結算（5 秒巡檢、重啟可接上）；
  賽事 ID 進網址可重整找回、分享連結加入、遊戲中賽事看板；接力依頁面分段、自動交棒；
  後台可設規則（限時 / 人數 / 分段，接力沒分段不能發佈）
- **身份規則**：訪客不上排行榜 / 不領獎勵，登入認領時補寫；場次 API 補參賽者檢查（既有 IDOR）
- **POS**：報表加「上個月 / 自訂區間」；首頁依權限顯示按鈕；兩個「今日結帳」改名（交班鎖帳 / 推送日報）；
  刪除的現金支出進垃圾桶可還原
- **後台防呆**：編輯器未存變更離開攔截、預覽讀最新內容、三個壞連結、重產 QR 先確認、取消預約原因必填、
  新增場域只給平台管理員、元件開關限本場域、補認 field_director、斷線寬限期改熱讀設定
- **發佈把關**：三條會改成已發佈的路徑共用同一套檢查（0 頁 / 類型不合法 / 接力沒分段擋下）
- **生產驗證**：`/api/version`=`22dff12c`、首頁與 `/api/health` 200、6 項安全標頭齊全、部署後 0 個 5xx、
  資料筆數與部署前一致（遊戲 45 / 場次 1230 / 排行榜 157 / 玩家 393 / 預約 109 / POS 支出 48 / 日結 80）、
  `games.match_config` 由啟動程序自動補上
### 🔒 分數偽造防線補強（部署後安全審查 hotfix）

**部署**：`96147b3f`（bundle `index-LtTQSBnX.js`）；部署前備份 `/opt/backups/db-gamehomicc-predeploy-20260923-015405.sql.gz`

- **H1 建立場次改白名單**：原本連 `score` / `status` / `hostMode` 都收 → 可直接開一個 9999 分或「已完成」的場次
- **H1 完成場次一律驗分數**：原本「沒帶 score 就跳過驗證」→ 先灌分、完成時只送 status 即可繞過（會污染全站排行榜）
- **H2 進度分數同步改走同一套驗證**（硬上限 + 射擊紀錄比對 + 可疑紀錄）：原本只有上限裁切，
  灌分後不完成、等時間到就以灌的分數結算
- **M5** 接力非當棒者送分 / 送完成一律不收；**M6** 接力沒分段時人數上限失效（可無限加入）
- **M7** 現金支出還原改要 `pos_cash_admin`（原本只要 `game:edit`，影響當日對帳）
- **M2** 公開排名顯示名不再退化成信箱前綴；**L1** 賽事暱稱補驗證；**L3** 倒數補救端點補限流
- **生產驗證**：`/api/version`=`96147b3f`、首頁與 health 200、6 項安全標頭、0 個 5xx、
  資料筆數不變（遊戲 45 / 場次 1230 / 排行榜 157 / 玩家 393 / 預約 109 / POS 支出 48 / 日結 80）；
  容器內確認新檢查已生效
...
**部署**：`e8d1d426`（bundle `index-B77q1NwH.js`）— 只含此修正（其餘未上線改動留在本地 main）

- **問題**：收款有整單折扣時，找零用折扣前金額計算（收 1000 折 200、客付 1000 → 畫面顯示找零 0，應找 200），
  確認收款按鈕、大金額確認對話框也顯示折扣前金額 → 收銀員少找錢。記帳金額由後端重算，原本就正確
- **修正**：找零 / 「剛好」/ 大金額門檻 / 按鈕與對話框金額一律用折扣後應收（純函式 `pos-checkout-math` + 6 案測試）
- **生產驗證**：`/api/version`=`e8d1d426`、health 200、6 項安全標頭、部署後無 5xx；線上 PosCheckout 程式檔確認為新算法
- 詳見 [changes/2026-09-23-platform-overhaul-plan.md](changes/2026-09-23-platform-overhaul-plan.md)（通盤規劃 P0）

---

## 2026-09-22

### 🎟️ 玩家動線通盤優化：免登入 + QR 直達 + 開局減法 + 結束保存紀錄 + 計分開關（feat + fix）

**部署**：`1e2237bc`（bundle `index-71Q3ADXy.js`）；部署前備份 `/root/db-backups/pre-deploy-player-flow-20260922-224455.sql.gz`
詳見 [changes/2026-09-22-player-flow-guest-qr-direct.md](changes/2026-09-22-player-flow-guest-qr-direct.md)

- **免登入**：遊玩路由自動建立訪客身分（GuestGate），拿掉登入牆 / 暱稱框；大廳、場域首頁直接進
- **QR 直達**：`/g/:slug` 過場頁直接導向（單人第一關 / 組隊大廳 / 章節）、套遊戲所屬場域；最壞 12 步 → 0 步；已印 QR 不用重印
- **開局減法**：直接接續＋可反悔提示、GPS 只給地點鎖、遊玩中不跳推薦隊伍 / PWA 卡 / 版本自動重整；
  組隊只隊長可開始、倒數 5→3 秒；修建立場次失敗無限重試
- **結束保存紀錄**：結算頁引導登入 → 訪客紀錄認領到正式帳號（HMAC 憑證、16 表、冪等、不刪資料）
- **計分開關** `games.scoring_enabled`（server 啟動冪等補欄位，已在生產自動補上）
- **附帶修**：競賽 / 接力開賽導向、隊伍獎勵帶 userId、QR 管理頁重產短連結
- **生產驗證**：`/api/version`=`1e2237bc`、6 項安全標頭齊全、資料筆數與部署前一致（遊戲 45 / 場次 1229 / 進度 1297 / 玩家 392 / 排行榜 157）、
  手機真瀏覽器未登入掃 QR → 直達第一關（無登入牆 / 繼續框）
...
**狀態**：🟡 開發與本機驗證完成，**尚未部署**（無 schema 變更）
詳見 [changes/2026-08-01-revenue-analytics.md](changes/2026-08-01-revenue-analytics.md)

- **🐛 修正總收入被誇大 100 倍**：POS 用「分」、遊戲/對戰用「元」，舊版直接相加
- **🐛 補上軟刪除排除**：已作廢的 POS 交易原本仍計入營收（含活動切片）
- **🐛 補上退款扣除**：原本完全沒扣 `refunds`，與 POS 報表對不起來
- **🐛 修正 timestamptz 時區轉換**：naive 與 tz 欄位轉換法不同，混用會差一天
- **🔑 釐清預約不是獨立收入源**：`pos.ts:916` 收款時會雙寫 bookings，
  列入營收等於同一筆錢算兩次 → 改以 `bookingId` 當維度切分
- **新增分析 API 5 端點**：summary（含環比）/ timeseries / breakdown（7 維度）/
  transactions / heatmap；場域隔離 + Zod 驗證 + 區間上限 400 天
- **`/admin/revenue` 改造**：移除「Phase 3 建置中」佔位，接上篩選列（6 快捷區間 +
  自訂日期 + 日/週/月）、KPI 環比、趨勢圖、來源占比、時段熱力圖、項目排行、
  交易明細與 CSV 匯出；篩選條件同步進 URL 可分享
- **色板經 CVD 驗證**（非目測）：兩模式 all-pairs 全通過；每個圖表都附表格檢視
- **測試**：真實 DB 整合 24 項 + 真實瀏覽器 e2e 16 項，全套 3310 passed
- **🐛 修正 POS 每日報表時區**：naive timestamp 少一層轉換，台北 08:00–16:00
  的交易被錯歸前一天。實測 128 筆中 9 筆錯歸、涉及 NT$12,530；修 10 處，
  另加 timezone-guard 守門測試（已實測植入錯誤會轉紅）
- **對帳實測**：舊畫面顯示 NT$13,746,400 vs 實際 NT$125,897（差 109 倍）；
  19 筆已付款預約 100% 都有對應 POS 交易，證實列入即重複計算
...
- 症狀：現場回報 6/29 結帳金額有誤，櫃台實際現金應 13,880 卻顯示 12,782（差 1,098）
- 根因：對交易「先退款後刪除」留下孤兒退款；報表/現金的退款加總只看 `refunds.status`，不檢查來源交易是否已刪除 → 憑空扣減營收與現金
- 修（C）：四處退款聚合加 `REFUND_SOURCE_NOT_DELETED`，排除來源 POS 交易已軟刪除的退款；既有錯誤資料自動失效，不需改生產 DB
- 修（B）：今日小結改顯示淨收款，有退款時並列「收/退」
- 修（A）：每筆顯示已退款金額、全額退款標「已退款」+劃線、退款鈕隱藏、退款前二次確認
- 驗證：生產 DB 試算 6/29 舊退款 1,098 → 修復後 0、收入 1,098；tsc 通過
- ⚠️ 後續：6/29 尚未最終結帳，請補做一次（修復後 variance=0、actualCash=13,880）
