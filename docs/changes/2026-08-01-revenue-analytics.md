...
具體案例：一筆 NT$597 實際發生在 7/26，日報表算成 7/25。

修正前後：

| 營業日 | 修正前 | 修正後 |
|--------|--------|--------|
| 7/25 | 4 筆 NT$2,675（多算隔日那筆） | 3 筆 NT$2,078 |
| 7/26 | 2 筆 NT$599（少算自己那筆） | 3 筆 NT$1,196 |

共修 10 處（9 處營業日 + 1 處 `EXTRACT(HOUR)` 時段分析），改用
`revenue-facts` 的 `taipeiBusinessDate()` / `taipeiLocalTime()`。
**保留未動 6 處**：`refunds.createdAt` 與 `bookings.slotStart` 是 timestamptz，
單層轉換本來就正確，多加一層反而會錯。

新增 `server/__tests__/timezone-guard.test.ts` 守門測試（純原始碼掃描、
不需 DB），並實測植入錯誤寫法會轉紅，確認不是永遠綠的假測試。
...
差 **109 倍**。另有 NT$11,567 是已軟刪除交易被錯誤計入。
目前沒有 completed 退款，故退款扣除為 0。

**② 預約重複計算已證實**

19 筆已付款預約，**19 筆全部**都有對應的 POS 交易（100% 重複），
總額 NT$23,028。若照原規劃把 bookings 列為第 4 個收入源，就會多算這筆。
...
1. ~~POS 報表時區 bug 未修~~ → **2026-08-01 已修**（見上方，實測 9 筆 / NT$12,530）
