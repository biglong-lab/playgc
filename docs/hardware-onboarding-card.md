...
## 08 · HMAC 簽章（防偽造命中）— 🆕 2026-08-06

> ⚠️ **順序很重要**：密鑰一旦發出，平台就會**強制驗簽**、未帶簽章的訊息一律拒收。
> 所以請照以下順序，不要提前索取密鑰（否則連通測試階段全部會被擋、很難除錯）。

1. 先完成 §06 連通測試 + §07 韌體清單（**不帶** `sig` 欄位，此時平台放行）
2. 韌體就緒後，通知平台方在後台按「Key」產生密鑰（**只顯示一次**，當場燒進韌體）
3. 之後每則 `event`（命中）訊息加 `sig` 欄位：

```
sig = HMAC-SHA256(secret, base) 的 hex 字串
base = "{deviceId}|{messageId}|{sentAt}|{zone}|{peak}"
```

4. 驗證：發一則正確簽章命中 → 後台看到計分；改壞一個字元 → 應被拒收
   （拒收原因可請平台方查 `/api/admin/mqtt/rejects`）

## 09 · 平台端已預先驗證（2026-08-06）

軟體全鏈已用模擬設備自動化驗證通過（10 案例）：契約驗證、跨場域拒收、
QoS1 重送去重、租約歸屬、伺服器計分、HMAC 強制驗簽。
**實測當天若不通，問題在韌體或網路層**，平台端可用拒收診斷直接定位。
