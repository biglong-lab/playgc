...
> **狀態**：✅ **遷移已完成**（2026-07-30 實測：`curl --resolve game.homi.cc:443:172.233.67.87`
> 回 HTTP 200，舊機 172.233.89.147 已無回應）。
>
> 本檔原為遷移前規劃，現轉為**日後搬遷／災難重建的參考**。下方兩個阻斷點與
> LiveKit 的 port 需求是永久有效的知識，不因這次遷移完成而失效 ——
> 尤其「對講機連得上但沒聲音」那個症狀，日後任何防火牆調整都可能重現。
>
> ⚠️ 本檔的步驟未經實際執行驗證（遷移是另外進行的），僅供參考。
