/**
 * E2E Smoke：template-market 7 情境 + 玩家端渲染
 *
 * 2026-06-13 重寫：原檔（874 行）建立在上一輪 loop 塞入的 112 情境 / 338 元件
 * 之上，248 個 toContain 斷言中有 217 個指向「沒有 renderer 的幽靈元件」。
 * 瘦身（只留 7 情境（2026-09-25 大螢幕情境移交 PhotoGo）、全部可渲染）後改為精簡 smoke：
 *   - /api/scenarios/health 回 7 情境、每情境 ≥1 元件
 *   - /template-market 顯示 7 張情境卡、無 JS 崩潰
 *   - 情境詳情頁可載入、列出元件
 *   - 玩家路由 /play、/g 可達不崩潰
 *
...
test.describe("template-market 7 情境 smoke", () => {
  ...
  test("/api/scenarios/health 回 7 情境、每情境 ≥1 元件", async ({ page }) => {
  ...
  test("/template-market 顯示 7 張情境卡、無崩潰", async ({ page }) => {
    const errors: string[] = [];
    page.on("pageerror", (err) => errors.push(err.message));
