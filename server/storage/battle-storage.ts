...
// ============================================================================
// 帶玩家名稱的查詢（JOIN users 表）
// ============================================================================

/** 取得排行榜並 JOIN 玩家名稱 */
async function getRankingsByFieldWithNames(fieldId: string, limit = 50) {
  const rows = await db
    .select({
...
/** 取得時段報名列表並 JOIN 玩家名稱 */
async function getRegistrationsBySlotWithNames(slotId: string) {
  const rows = await db
    .select({
...
/** 取得戰隊成員列表並 JOIN 玩家名稱 */
async function getClanMembersWithNames(clanId: string) {
  const rows = await db
    .select({
...
async function getPlayerResultsByResultWithNames(resultId: string) {
  const rows = await db
    .select({
...
async function getPlayerHistoryWithDetails(userId: string, limit = 20) {
  const rows = await db
    .select({
      playerResult: battlePlayerResults,
...
async function getUpcomingRegistrationsWithDetails(userId: string) {
  const rows = await db
    .select({
      registration: battleRegistrations,
      slotDate: battleSlots.slotDate,
