...
//
// 🆕 2026-09-23：由 client/src/pages/game-editor/lib/validate-page-config.ts 搬來（純函式、行為不變），
//   讓後端發佈端點也能跑同一套檢查（前後端共用）；前端舊路徑保留為 re-export。
//   只依賴 PageLike 結構型別，不 import schema（避免把 drizzle 表定義拉進前端 / 測試 mock 衝突）。

/** 驗證所需的最小頁面形狀（DB 的 pages 列與前端 Page 都相容） */
